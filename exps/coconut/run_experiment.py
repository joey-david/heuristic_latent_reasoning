...
def simulate_answer(prompt: str, num_latent_thoughts: int) -> Tuple[str, int, str]:
    fake_answer = str((len(prompt) + num_latent_thoughts) % 23)
    fake_tokens = len(prompt.split()) + 25 + num_latent_thoughts
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the Coconut-style GPT-2 model and log metrics."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="How many latent refinement steps the Coconut model should use.",
    )
    ...
    logger = ExperimentLogger(log_path, model_name="coconut", run_id=args.run_id)
    ...
    for idx, problem in enumerate(dataset):
        ...
        if args.dry_run:
            predicted, tokens, raw_completion = simulate_answer(
                question, args.num_latent_thoughts
            )
        else:
        ...
        extras = {
            ...
            "configured_latent_passes": args.num_latent_thoughts,
        }
